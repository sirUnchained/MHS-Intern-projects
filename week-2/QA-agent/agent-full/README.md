...
The graph classifies every user message as either answerable by the Main Agent (using RAG,
financial data, or web search) or needing escalation to a specific department. The Main Agent can
call tools repeatedly up to `MAX_TOOL_CALLS`; once the limit is reached, it must answer with what
it already has. Every response is validated for sensitive content (PII, URLs) – if validation
fails, the conversation is redirected to ticket creation. After a substantive exchange, a
lightweight extraction node decides whether any long‑term memory should be saved to the
Postgres‑backed LangGraph store for future conversations.
...
| `MARKET_DATA_REFRESH_HOUR_UTC` | `0` | Hour (0–23 UTC) at which a scheduled market data refresh would run, if a scheduler/cron is configured. Not used directly by the web app. |
